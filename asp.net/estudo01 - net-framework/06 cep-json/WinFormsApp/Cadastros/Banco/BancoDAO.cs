    ...
    public class BancoDAO : ConexaoSqlite
    {
        ...
        public IEnumerable<Banco> Listar(int idFiltro, string valor)
        {
            ...
            try
            {
                // select basico.
                //************************************************
                string cQryBase = "SELECT * FROM dcbanco ";
        ...
        public Banco BuscarID(string Codigo)
        {
            ...
            return db.Query<Banco>(@"SELECT * FROM dcbanco WHERE Codigo = @pCodigo", new { pCodigo = Codigo }).SingleOrDefault();
        ...
        {
            using var db = ConexaoDB;
            return db.Execute(@"DELETE FROM dcbanco WHERE Codigo = @pCodigo", new { pCodigo = Codigo });
        ...
        public int Salvar(OperacaoEnum operacao, Banco banco)
        {
            try
            {
                ...
                {
                        //zero é inclusão
                        return db.Execute(@"INSERT INTO dcbanco(codigo, nome, fantasia, idempresa) 
                                            VALUES (@Codigo, @Nome, @fantasia, @IdEmpresa);", banco);

                }
                else
                {
                       return  db.Execute(@"UPDATE dcbanco SET Nome = @Nome, Fantasia = @Fantasia, 
                                                            IdEmpresa = @IdEmpresa 
